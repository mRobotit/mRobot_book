...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2020-11-21
 @ Function     -> Mrobotit_start_object()
***/

Mrobotit_start_object::Mrobotit_start_object()
{
	...
	nh_private.param<std::string>("usart_port", this->usart_port, "/dev/mrobotit_base"); 
	...
	this->cmd_vel_sub = n.subscribe(smoother_cmd_vel, 100, &Mrobotit_start_object::cmd_velCallback, this);
	this->amcl_sub = n.subscribe("/amcl_pose", 100, &Mrobotit_start_object::cmd_AmclvelCallback, this);
    ...
    this->scan_sub = n.subscribe<sensor_msgs::LaserScan> ("/scan", 100, &Mrobotit_start_object::cmd_velScanCallback, this);
        this->scan_pub  = n.advertise<sensor_msgs::LaserScan> ("/scan_filter", 100, false);
	/**open seril device**/
	try{
		 ...
		 ROS_ERROR_STREAM("[MROBOTIT] Unable to open port ");
	}
	if(Robot_Serial.isOpen()){
	 	ROS_INFO_STREAM("[MROBOTIT] Serial Port opened");
	}else{
	}
}
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2020-11-21
 @ Function     -> Mrobotit_start_object::cmd_velCallback(const geometry_msgs::Twist &twist_aux)
***/
void Mrobotit_start_object::cmd_velCallback(const geometry_msgs::Twist &twist_aux)
{
...
void Mrobotit_start_object::cmd_velScanCallback(const sensor_msgs::LaserScan::ConstPtr& scan){

	   sensor_msgs::LaserScan scan_msg;
	int node_count  =   scan->intensities.size();
    scan_msg.header.stamp = scan->header.stamp;
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2020-11-21
 @ Function     ->  Mrobotit_start_object::cmd_AmclvelCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr &amclPose)
***/
void Mrobotit_start_object::cmd_AmclvelCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr &Pose)
{
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2020-11-21
 @ Function     -> void Mrobotit_start_object::PublisherOdom()
***/
void Mrobotit_start_object::PublisherOdom()
{
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2020-11-21
 @ Function     -> bool Mrobotit_start_object::ReadFromUart()
 @ return 		-> status
***/
bool Mrobotit_start_object::ReadFromUart()
{	
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2019-03-25
 @ Function     -> void Mrobotit_start_object::publisherImuSensorRaw()
 @ return 		-> null
***/
void Mrobotit_start_object::publisherImuSensorRaw()
{
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2019-03-25
 @ Function     -> void Mrobotit_start_object::publisherImuSensor()
 @ return 		-> null
***/
void Mrobotit_start_object::publisherImuSensor()
{
	...
	//among this range ,angular_velocity would be zero.
	// if((Mpu6050.angular_velocity.z >= GRYOSCOPE_ZERO_MIN)&(Mpu6050.angular_velocity.z <= GRYOSCOPE_ZERO_MAX ))
	// {
	// 	ImuSensor.angular_velocity.z = 0.0;
	// }
	// else{
	// 	ImuSensor.angular_velocity.z = Mpu6050.angular_velocity.z;
	// }
...
/***
 ...
 @ Author		-> Mrobotit
 @ Date			-> 2020-11-21
 @ Function     -> bool Mrobotit_start_object::ReadAndWriteLoopProcess()
 @ return 		-> status
***/
bool Mrobotit_start_object::ReadAndWriteLoopProcess()
{
	...
	while(ros::ok())
	{
		...
		if (true == ReadFormUart()) 	/* Get npu data include robot move speed and action status information*/
		{
			...
			else
			{
				...
				MahonyAHRSupdateIMU(Mpu6050.angular_velocity.x, Mpu6050.angular_velocity.y, Mpu6050.angular_velocity.z,\
...
float Mrobotit_start_object::invSqrt(float number)
{
	volatile long i;
    volatile float x, y;
...
void Mrobotit_start_object::accelerometerOffset(float gx, float gy, float gz)
{
...
void Mrobotit_start_object::MahonyAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az)
{
	float recipNorm;
...
int main(int argc, char** argv)
{
	...
	ROS_INFO("[MROBOTIT] base controller node start! ");

	Mrobotit_start_object Robot_Control; 
